...
- **皮肤 v4**：以用户参考图为蓝本、图生图模式高保真重制六态立绘
  （覆盖 v3，git 历史留档），分辨率提升至 **768×1344**：
  - 服装严格对齐参考图：黑色蕾丝丝绸胸衣 + 蓬松黑丝绸肩袖 + 白色紧袖
    蕾丝蓬松袖口 + 黑丝带白蕾丝项圈 + 白色高腰百褶裙（黑边）+
    **黑色蕾丝过膝袜（绝对领域）** + 黑色玛丽珍小皮鞋；
  - 淡紫银及膝长发；**美人痣仅一颗、隐约可见**；人物正面朝向镜头；
  - 各状态具备**不同肢体动作**（不只表情）：happy 双手举脸旁猫爪、
    pout 嘟嘴抱臂、angry 叉腰前倾、sleepy 双手揉眼含泪、blink 双眼全闭；
  - 管线升级：idle 由参考图 img2img 放大重绘 → 差分 txt2img +
    ControlNet reference_only 锁角色服装 → ADetailer 双模型（脸+手）
    防手指崩坏 → **眨眼帧眼部定点 inpaint**（多种子择优）强制双眼全闭
    → 边缘泛洪预抠透明；手指/发尾负面词强约束。
- **新增 `docs/技术文档.md`**：八章全景技术文档——总体架构、视觉检测层
  （EAR/单目测距/CVA 公式与关键点）、核心算法层（情绪 AU 映射/建议分级/
  守护判据/人格聊天/LLM 客户端）、健康报告体系、桌宠技术栈、
  **皮肤 AIGC 创作管线 v4**、辅助工程能力、配置阈值速查表。
- 版本号提升至 1.19.0-demo（config/README 同步）。

---

## v1.18.0-demo — 2026-07-22

### 变更（弥悠官方皮肤重制 v3，2026-07-22）
- 依据用户反馈全面重制 `assets/skins/miyou/` 六态立绘（覆盖旧版，git 历史留档）：
  白色蕾丝花边连衣裙 + 黑色蝴蝶结 + 分离式泡泡大袖 + **白色过膝袜（绝对领域）**
  + **黑色日系玛丽珍小皮鞋**，六张服装/鞋完全一致；**浅粉紫渐变瞳（较旧版调浅、
  去凶感）**、短毛猫耳、黑色爱心发夹、无铃铛项圈、右眼下小美人痣；B 罩杯身形；
  神态慵懒放松调皮可爱，各差分加入符合情绪的肢体动作与神情。
- 针对性修复用户三点意见：① 头发**去除粉色线描轮廓**（负面词强约束）；
  ② 丝袜由黑改**白色过膝袜**并显出绝对领域（对抗模型黑袜偏好，白袜权重提至 1.7）；
  ③ 全身像画布加高至 512×896，猫耳/鞋子不截断、人物大小统一。
- 制作管线：txt2img 定 idle 基准 → img2img 低幅差分（denoise 0.42）保服装一致
  → ADetailer 修脸（浅粉紫渐变瞳 + 美人痣）→ 预抠图直出 RGBA
  （纯灰背景泛洪 + 阈值收紧避免误抠人物，`pet.py` 检测到自带 alpha 直接采用）。
- 历史版本补发：v1.10~v1.15 补 git tag，v1.10~v1.17 全部建 GitHub
  Release 并附源码 zip（v1.15+ 加安装包），仓库进入完整 Release 制。

### 变更（弥悠身世升级 + 双模式 LLM 实时台词，2026-07-22）
...
- **聊天模式**（`core/chat_engine.py`）：`_system_prompt` 注入完整 D+A 身世与
  观星口癖，叠加好感度档位语气，离线台词池 name/privacy 场景同步更新。
- **非聊天模式**（`core/persona.py`）：后台台词扩充线程 `refresh_lines_llm`
  的系统提示词同步注入 D+A 身世，让桌宠气泡（用眼提醒/夸奖/情绪关怀）也由 LLM
  按身世实时生成、混入随机抽取池；由 `monitor.py` 延迟启动，LLM 不可用时零影响
  （离线兜底铁律）。两模式台词风格自此统一。
