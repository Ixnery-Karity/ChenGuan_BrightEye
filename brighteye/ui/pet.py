...
# 皮肤隔离规则（v1.17.0）：优先加载 assets/skins/<CONFIG.skin>/ 下的
# 通用命名整套皮肤（idle.png 存在即整套生效，缺的情绪回退本皮肤 idle，
# **绝不与旧 assets/pet/wenna_* 混搭**，避免新旧形象串戏）；
# 无任何皮肤时才回退旧文乃立绘目录，再无则矢量绘制。详见 assets/skins/README.txt。
ASSET_DIR = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "assets", "pet"))
SKINS_DIR = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "assets", "skins"))
SPRITE_FILES = {
...
SKIN_BLINK_FILES = {
    NORMAL: "blink.png",
    HAPPY: "happy_blink.png",
    POUT: "pout_blink.png",
    ANGRY: "angry_blink.png",
}


def _active_skin_dir() -> Optional[str]:
    """返回激活皮肤目录（含 idle.png 才算有效整套），无则 None。"""
    try:
        from ..config import CONFIG
        name = getattr(CONFIG, "skin", "") or ""
    except Exception:
        name = ""
    if not name:
        return None
    d = os.path.join(SKINS_DIR, name)
    if os.path.isfile(os.path.join(d, "idle.png")):
        return d
    return None
...
def _load_sprites_impl(pil_only: bool = False):
    paths: Dict[str, str] = {}
    skin_dir = _active_skin_dir()
    if skin_dir:
        # —— 皮肤隔离：整套只从激活皮肤目录取，绝不混入旧 wenna_* ——
        for mood, fname in SKIN_SPRITE_FILES.items():
            p = os.path.join(skin_dir, fname)
            if os.path.isfile(p):
                paths[mood] = p
        for mood, fname in SKIN_BLINK_FILES.items():
            p = os.path.join(skin_dir, fname)
            if os.path.isfile(p):
                paths["blink_" + mood] = p
    else:
